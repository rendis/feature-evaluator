...
func (r *SecurityPolicyRepo) Get(ctx context.Context) (*securitypolicy.ManagedPolicy, error) {
	row := r.client.db(ctx).QueryRow(ctx, `
		SELECT cors_origins, external_api_allow_hosts, updated_at, updated_by
	...
	if err := decodeJSON(allowHostsJSON, &policy.ExternalAPIAllowHosts); err != nil {
		return nil, fmt.Errorf("decode security policy external api allow hosts: %w", err)
	}
...
func (r *SecurityPolicyRepo) Upsert(ctx context.Context, policy *securitypolicy.ManagedPolicy) error {
	...
	allowHostsJSON, err := jsonBytes(policy.ExternalAPIAllowHosts, "[]")
	if err != nil {
		return fmt.Errorf("marshal security policy external api allow hosts: %w", err)
	}

	if _, err := r.client.db(ctx).Exec(ctx, `
		INSERT INTO system_security_policies (
			singleton_key, cors_origins, external_api_allow_hosts, updated_at, updated_by
		) VALUES ($1, $2::jsonb, $3::jsonb, $4, $5)
		ON CONFLICT (singleton_key) DO UPDATE
		SET cors_origins = EXCLUDED.cors_origins,
		    external_api_allow_hosts = EXCLUDED.external_api_allow_hosts,
