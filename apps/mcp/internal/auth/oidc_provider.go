...
const (
	...
	configDirName  = ".feature-evaluator"
...
// OIDCTokenProvider reads tokens from ~/.feature-evaluator/tokens.json and
...
// is missing — callers should prompt the user to run `feature-evaluator-mcp login`.
func NewOIDCTokenProvider(filePath string) (*OIDCTokenProvider, error) {
	tokens, err := loadTokens(filePath)
	if err != nil {
		return nil, fmt.Errorf("no stored tokens (run `feature-evaluator-mcp login` first): %w", err)
	}
...
func (p *OIDCTokenProvider) Token(ctx context.Context) (string, error) {
	...
	if p.tokens.RefreshToken == "" {
		return "", fmt.Errorf("access token expired and no refresh token available — run `feature-evaluator-mcp login`")
	}

	if err := p.refresh(ctx); err != nil {
		return "", fmt.Errorf("token refresh failed (run `feature-evaluator-mcp login`): %w", err)
	}
	return p.tokens.AccessToken, nil
}
